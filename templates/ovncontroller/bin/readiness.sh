...
check_ovsdb_server() {
    ...
    if [ ! -e $ovsdbserverpidfile ];  then
        error_exit "ERROR - ovsdb-server PID file does not exist"
    fi
...
check_ovs_vswitchd() {
    ...
    if [ ! -e $ovsvswitchdpidfile ]; then
        error_exit "ERROR - ovs-vswitchd PID file does not exist"
    fi
