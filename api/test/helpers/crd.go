...
// CreateOVNNorthd creates a new OVNNorthd instance with the specified name and
// namespace in the Kubernetes cluster. If name is not passed (nil) it will be
// autogenerated
//
// Example usage:
//
//	ovnNorthd := th.CreateOVNNorthd(nil, namespace, spec)
//	DeferCleanup(th.DeleteOVNNorthd, ovnNorthd)
func (th *TestHelper) CreateOVNNorthd(northName *string, namespace string, spec ovnv1.OVNNorthdSpec) types.NamespacedName {
...
// name and namespace in the Kubernetes cluster. If name is not passed (nil),
// it will be autogenerated.
//
// Example usage:
//
//	ovnDBCluster := th.CreateOVNDBCluster(nil, namespace, spec)
//	DeferCleanup(th.DeleteOVNDBCluster, ovnDBCluster)
func (th *TestHelper) CreateOVNDBCluster(ovnDBname *string, namespace string, spec ovnv1.OVNDBClusterSpec) types.NamespacedName {
...
// Example usage:
//
//	ovnController := th.CreateOVNController(nil, namespace, spec)
//	DeferCleanup(th.DeleteOVNController, ovnController)
func (th *TestHelper) CreateOVNController(ctrlName *string, namespace string, spec ovnv1.OVNControllerSpec) types.NamespacedName {
...
// CreateOVNDBBackup creates a new OVNDBBackup instance with the specified
// name and namespace in the Kubernetes cluster. If name is not passed (nil),
// it will be autogenerated.
func (th *TestHelper) CreateOVNDBBackup(backupName *string, namespace string, spec ovnv1.OVNDBBackupSpec) types.NamespacedName {
	name := "ovndbbackup-" + uuid.New().String()
	if backupName != nil {
		name = *backupName
	}
	ovnDBBackup := &ovnv1.OVNDBBackup{
		TypeMeta: metav1.TypeMeta{
			APIVersion: "ovn.openstack.org/v1beta1",
			Kind:       "OVNDBBackup",
		},
	...
	gomega.Expect(th.K8sClient.Create(th.Ctx, ovnDBBackup)).Should(gomega.Succeed())
	th.Logger.Info("OVNDBBackup created", "OVNDBBackup", name)
	return types.NamespacedName{Namespace: namespace, Name: name}
}

// DeleteOVNDBBackup deletes an OVNDBBackup resource from the Kubernetes cluster.
func (th *TestHelper) DeleteOVNDBBackup(name types.NamespacedName) {
	gomega.Eventually(func(g gomega.Gomega) {
		ovnDBBackup := &ovnv1.OVNDBBackup{}
		err := th.K8sClient.Get(th.Ctx, name, ovnDBBackup)
		if k8s_errors.IsNotFound(err) {
			return
		}
		g.Expect(err).NotTo(gomega.HaveOccurred())

		g.Expect(th.K8sClient.Delete(th.Ctx, ovnDBBackup)).Should(gomega.Succeed())
...
// GetOVNDBBackup retrieves an OVNDBBackup resource.
func (th *TestHelper) GetOVNDBBackup(name types.NamespacedName) *ovnv1.OVNDBBackup {
...
// SimulateOVNDBBackupReady simulates the readiness of an OVNDBBackup resource.
func (th *TestHelper) SimulateOVNDBBackupReady(name types.NamespacedName) {
	gomega.Eventually(func(g gomega.Gomega) {
		backup := th.GetOVNDBBackup(name)
		backup.Status.ObservedGeneration = backup.Generation
		backup.Status.Conditions.MarkTrue(condition.ReadyCondition, "Ready")
		g.Expect(th.K8sClient.Status().Update(th.Ctx, backup)).To(gomega.Succeed())
	}, th.Timeout, th.Interval).Should(gomega.Succeed())
	th.Logger.Info("Simulated OVNDBBackup ready", "on", name)
}

// CreateOVNDBRestore creates a new OVNDBRestore instance with the specified
// name and namespace in the Kubernetes cluster. If name is not passed (nil),
// it will be autogenerated.
func (th *TestHelper) CreateOVNDBRestore(restoreName *string, namespace string, spec ovnv1.OVNDBRestoreSpec) types.NamespacedName {
	name := "ovndbrestore-" + uuid.New().String()
	if restoreName != nil {
		name = *restoreName
	}
	ovnDBRestore := &ovnv1.OVNDBRestore{
		TypeMeta: metav1.TypeMeta{
			APIVersion: "ovn.openstack.org/v1beta1",
			Kind:       "OVNDBRestore",
		},
	...
	gomega.Expect(th.K8sClient.Create(th.Ctx, ovnDBRestore)).Should(gomega.Succeed())
	th.Logger.Info("OVNDBRestore created", "OVNDBRestore", name)
	return types.NamespacedName{Namespace: namespace, Name: name}
}

// DeleteOVNDBRestore deletes an OVNDBRestore resource from the Kubernetes cluster.
func (th *TestHelper) DeleteOVNDBRestore(name types.NamespacedName) {
	gomega.Eventually(func(g gomega.Gomega) {
		ovnDBRestore := &ovnv1.OVNDBRestore{}
		err := th.K8sClient.Get(th.Ctx, name, ovnDBRestore)
		if k8s_errors.IsNotFound(err) {
			return
		}
		g.Expect(err).NotTo(gomega.HaveOccurred())

		g.Expect(th.K8sClient.Delete(th.Ctx, ovnDBRestore)).Should(gomega.Succeed())
...
// GetOVNDBRestore retrieves an OVNDBRestore resource.
func (th *TestHelper) GetOVNDBRestore(name types.NamespacedName) *ovnv1.OVNDBRestore {
